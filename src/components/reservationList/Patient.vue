    ...
    <v-card light style="height:100%">
      ...
      <v-navigation-drawer permanent style="height:90%;">
        ...
        <div v-else-if="active==2">
          <v-card v-for="(reservation,index) in isFinished(reservations)" :key="index">
              <v-menu :key="reservation.patient.name" full-width offset-x left>
          ...
          <v-card v-if="isFinished(reservations).length<1">
            <v-card-text>No Appointment</v-card-text>
          </v-card>
        </div>
      </v-navigation-drawer>
...
export default {
  ...
  methods: {
    ...
    isFinished(arr) {
         let finishArr = arr.filter(function(e) {
