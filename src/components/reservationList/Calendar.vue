          ...
          <v-flex sm3>
            <div style="height:100%">
              ...
              <div style="margin-top: 15%;height:73.8% ">
                <patient :reservations="computedNewArr(doctor.reservations)"/>
              </div>
            </div>
          </v-flex>
        </v-layout>
...
import EditReservationDialog from "@/components/dialog/editReservationDialog";
...
export default {
  ...
  watch: {
    ...
    isEdit: function(val) {
      if (val) {
        this.$apollo.queries.doctor.refetch();
        this.actionSetIsEditFromReservationList(false);
      }
    },
  ...
  methods: {
    ...mapActions([
      ...
      "actionSetRefreshNow",
      "actionSetIsEditFromReservationList"
    ]),
